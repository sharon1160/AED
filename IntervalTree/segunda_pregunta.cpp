...
// Devuelve un intervalo superpuesto i que tiene el low mínimo
Interval * minIntervalSearch(ITNode *root, Interval i) {
    ITNode *x = root;
    while (x != NULL) {
...
{ 
	...
	Interval *respuesta = overlapSearch(root, x); 
	cout<<"Se superpone con ["<< respuesta->low << ", " << respuesta->high << "]"<<" que tiene el valor low mínimo o NULL"<<endl;;
	return 0; 
} 
